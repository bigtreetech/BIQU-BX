...
#if HAS_LCD_MENU || ENABLED(HOST_PROMPT_SUPPORT)
...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  while (thermalManager.degTargetHotend(active_extruder) != 0 && ABS(thermalManager.degHotend(active_extruder) - thermalManager.degTargetHotend(active_extruder)) > TEMP_WINDOW)
    idle();
...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (fast_load_length) {
    ...
    do {
      ...
      #if HAS_LCD_MENU || (ENABLED(HOST_PROMPT_SUPPORT) && ENABLED(ADVANCED_PAUSE_FEATURE))
        if (show_lcd) {
