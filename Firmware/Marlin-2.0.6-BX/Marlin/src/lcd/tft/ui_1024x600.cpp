...
void draw_heater_status(uint16_t x, uint16_t y, const int8_t Heater) {
  ...
  TERN_(TOUCH_SCREEN, if (targetTemperature >= 0) touch.add_control(HEATER, x, y, 120, 120, Heater));
  tft.canvas(x, y, 120, 120);
  tft.set_background(COLOR_BACKGROUND);
...
void draw_fan_status(uint16_t x, uint16_t y, const bool blink) {
  TERN_(TOUCH_SCREEN, touch.add_control(FAN, x, y, 120, 120));
  tft.canvas(x, y, 120, 120);
  tft.set_background(COLOR_BACKGROUND);
...
void MarlinUI::draw_status_screen() {
  ...
  // coordinates
  tft.canvas(4, 200, TFT_WIDTH - 8, 34);
  tft.set_background(COLOR_BACKGROUND);
  ...
  tft.add_text(300 - tft_string.width(), 3, is_homed ? COLOR_AXIS_HOMED : COLOR_AXIS_NOT_HOMED, tft_string);

  is_homed = TEST(axis_homed, Y_AXIS);
  tft_string.set(blink & !is_homed ? "?" : ftostr4sign(LOGICAL_Y_POSITION(current_position.y)));
  tft.add_text(600 - tft_string.width(), 3, is_homed ? COLOR_AXIS_HOMED : COLOR_AXIS_NOT_HOMED, tft_string);
  ...
  tft.add_text(900 - tft_string.width() - offset, 3, is_homed ? COLOR_AXIS_HOMED : COLOR_AXIS_NOT_HOMED, tft_string);

  // feed rate
  tft.canvas(274, 300, 100, 32);
  tft.set_background(COLOR_BACKGROUND);
  ...
  tft.canvas((TFT_WIDTH - 128) / 2, 400, 128, 29);
  tft.set_background(COLOR_BACKGROUND);
  ...
  tft.canvas(4, 450, TFT_WIDTH - 8, 9);
