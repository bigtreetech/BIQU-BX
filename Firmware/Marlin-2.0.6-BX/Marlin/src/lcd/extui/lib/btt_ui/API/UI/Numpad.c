...
const GUI_RECT oldParameterRect = {0,                        0, BTT_LCD_WIDTH/2 - BYTE_WIDTH,  ICON_START_Y+0*SKEYHEIGHT};
const GUI_RECT newParameterRect = {BTT_LCD_WIDTH/2 + BYTE_WIDTH, 0,                BTT_LCD_WIDTH,  ICON_START_Y+0*SKEYHEIGHT};
const GUI_RECT arrowRect        = {BTT_LCD_WIDTH/2 - BYTE_WIDTH, 0, BTT_LCD_WIDTH/2 + BYTE_WIDTH,  ICON_START_Y+0*SKEYHEIGHT};
...
void Draw_keyboard(uint8_t * title, bool NumberOnly, bool negative)
{
    ...
    GUI_ClearRect(0, 0, BTT_LCD_WIDTH, rect_of_numkey[0].y0);
    GUI_ClearRect(0, rect_of_numkey[0].y0, BTT_LCD_WIDTH, BTT_LCD_HEIGHT);
...
float numPadFloat(uint8_t* title, float old_val, float reset_val, bool negative)
{
  ...
  menuSetFrontCallBack(NULL);           //Cancel callback make sure touch is normal
  while (1)
  {
