...
const uint8_t  pidDegree[ITEM_PID_DEGREE_NUM] = {1, 5, 10};
static uint8_t curDegree = 1;
...
void pidUpdateStatus(bool succeeded)
{
  ...
  else                                 // if all the PID processes terminated, provide the final dialog
  {
    ...
    if (pidSucceeded)                  // if all the PID processes successfully terminated, allow to save to EEPROM
    {
      ...
      showDialog(DIALOG_TYPE_SUCCESS, textSelect(LABEL_PID_TITLE), (uint8_t*) tmpBuf,
...
//      pidSucceeded = false;          // pidUpdateStatus function allow to handle status updates eventually arriving after the timeout
      ...
      popupReminder(DIALOG_TYPE_ERROR, textSelect(LABEL_PID_TITLE), (uint8_t*) tmpBuf);
    }
  }
}
...
static bool isPressed, isReleased;
void menuPidWaitCallBack(void)
{
...
void menuPidWait(void)
{
  ...
  GUI_DispStringInRectEOL(20, BYTE_HEIGHT * 7, BTT_LCD_WIDTH - 20, BTT_LCD_HEIGHT, textSelect(LABEL_PID_START_INFO_3));
  menuSetFrontCallBack(menuPidWaitCallBack);
}
  ...
  KEY_VALUES key_num = menuKeyGetValue();
    switch (key_num)
    {
      ...
      case KEY_ICON_4:
        ...
        while (!heaterIsValid(pidHeater.toolIndex) || pidHeater.toolIndex == TSC_CHAMBER);
...
void menuPid(void)
{
  ...
  pidTemperatureReDraw(false);
  menuSetFrontCallBack(menuPidCallBack);
}
