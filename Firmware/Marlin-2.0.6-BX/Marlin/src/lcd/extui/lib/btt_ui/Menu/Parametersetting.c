...
const LISTITEMS parameter_menuitems_init_value ={
// title
LABEL_BACKGROUND, //Bad initialization!!
// icon           ItemType        Item Title      item value text(only for custom value)
...
static PARAMETERS now;
//show menu for selected parameter type
void menuShowParameterCallBack(void)
{
  ...
  default:
    if (key_num < (KEY_VALUES)STEPPER_COUNT)
    {
      ...
      menuSetFrontCallBack(menuShowParameterCallBack);
    }
    break;
  }
...
{
  memcpy(&parameter_menuitems, &parameter_menuitems_init_value, sizeof(LISTITEMS));
  parameter_menuitems.title =  parametertypes[cur_parameter].titlelabel;
  ...
  for (int i = 0; i < getParameterElementCount(cur_parameter); i++) {
    ...
    case P_ACCELERATION:
      ...
      break;
    case P_JERK:
      setDynamicLabel(TSC_X_AXIS, "X");
      setDynamicLabel(TSC_Y_AXIS, "Y");
      setDynamicLabel(TSC_Z_AXIS, "Z");
      setDynamicLabel(TSC_E_AXIS, "E");
      break;
    case P_FWRETRACT:
...
void menuParameterSettingsCallBack(void)
{
  KEY_VALUES key_num = menuKeyGetValue();
  switch (key_num)
  {
...
void menuParameterSettings(void)
{
  ...
  menuSetFrontCallBack(menuParameterSettingsCallBack);
}
...
bool temperatureStatusValid(void)
{
  ...
  // if (infoMenu.menu[infoMenu.cur] == menuManualLeveling) return false;
  if (infoMenu.menu[infoMenu.cur] == menuSendGcode) return false;
...
void loopTemperatureStatus(void)
{
  ...
  if (infoSettings.bed_en) { // global bed
    tmpHeater[tmpIndex++] = TSC_BED;
  }
  if (infoSettings.chamber_en) { // global chamber
    tmpHeater[tmpIndex++] = TSC_CHAMBER;
  }
...
{
  int16_t x_offset = BTT_LCD_WIDTH - 10;
