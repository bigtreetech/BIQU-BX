...
#define TSC_SIGN  0x20200923 // DO NOT MODIFY
#define PARA_SIGN 0x20201111 // (YYYMMDD) If a new setting parameter is added,
...
void floatToByte(float value, uint8_t *bytes)
{
  memcpy(bytes, &value, 4);
}

float byteToFloat(uint8_t *bytes, uint8_t len)
{
  float word = 0.0f;
...
{
  uint8_t data[INFO_SETTINGS_MAX_SIZE];
  uint32_t index = 0;
  uint32_t sign = 0;
  W25Qxx_ReadBuffer(data, INFO_SETTINGS_ADDR, INFO_SETTINGS_MAX_SIZE);

  sign = byteToWord(data + (index += 4), 4);
  if(sign == TSC_SIGN)
  {
    if(byteToWord(data + index + 4, 4) != (0xffffffff)) paraStatus |= PARA_TSC_EXIST;    // If the touch screen calibration parameter already exists
    tsc_x_inc    = byteToFloat(data + (index += 4), 4);
    tsc_x_offset = byteToFloat(data + (index += 4), 4);
    tsc_y_inc    = byteToFloat(data + (index += 4), 4);
    tsc_y_offset = byteToFloat(data + (index += 4), 4);
    infoSettings.rotate_ui            = byteToWord(data + (index += 4), 4); // rotate_ui and TSC_Para[] are bound together
  }
  else
  {
    infoSettings.rotate_ui            = _ENABLED;
  }
...
//machine specific settings
  ...
  infoSettings.pause_retract_len      = byteToFloat(data + (index += 4), 4);
  infoSettings.resume_purge_len       = byteToFloat(data + (index += 4), 4);


  infoSettings.pause_pos[TSC_X_AXIS]  = byteToFloat(data + (index += 4), 4);  // X
  infoSettings.pause_pos[TSC_Y_AXIS]  = byteToFloat(data + (index += 4), 4);  // Y
  infoSettings.pause_z_raise          = byteToFloat(data + (index += 4), 4);
  ...
  infoSettings.level_z_pos            = byteToFloat(data + (index += 4), 4);
  infoSettings.level_z_raise          = byteToFloat(data + (index += 4), 4);
...
void storePara(void)
{
  ...
  floatToByte(tsc_x_inc,                              data + (index += 4));
  floatToByte(tsc_x_offset,                           data + (index += 4));
  floatToByte(tsc_y_inc,                              data + (index += 4));
  floatToByte(tsc_y_offset,                           data + (index += 4));
  ...
  floatToByte(infoSettings.pause_retract_len,         data + (index += 4));
  floatToByte(infoSettings.resume_purge_len,          data + (index += 4));


  floatToByte(infoSettings.pause_pos[TSC_X_AXIS],     data + (index += 4));  // X
  floatToByte(infoSettings.pause_pos[TSC_Y_AXIS],     data + (index += 4));  // Y
  floatToByte(infoSettings.pause_z_raise,             data + (index += 4));
  ...
  floatToByte(infoSettings.level_z_pos,               data + (index += 4));
  floatToByte(infoSettings.level_z_raise,             data + (index += 4));
  ...
  W25Qxx_EraseSector(INFO_SETTINGS_ADDR);
  W25Qxx_WriteBuffer(data, INFO_SETTINGS_ADDR, INFO_SETTINGS_MAX_SIZE);
}
