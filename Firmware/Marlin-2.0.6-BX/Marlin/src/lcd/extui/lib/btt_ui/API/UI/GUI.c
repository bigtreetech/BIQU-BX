...
{
  LTDC_Draw_Point(x, y, foreGroundColor);
}

void GUI_FillRect(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey)
{
  LTDC_Fill(sx, sy, ex, ey, foreGroundColor);
}
...
{
  LTDC_Fill(sx, sy, ex, ey, backGroundColor);
}
...
void GUI_HLine(uint16_t x1, uint16_t y, uint16_t x2)
{
  ...
  for(i=x1; i<x2; i++)
  {
    LTDC_Draw_Point(i, y, foreGroundColor);
  }
}
void GUI_VLine(uint16_t x, uint16_t y1, uint16_t y2)
{
  ...
  for(i=y1; i<y2; i++)
  {
    LTDC_Draw_Point(x, i, foreGroundColor);
  }
}
...
void  GUI_FillCircle(uint16_t x0, uint16_t y0, uint16_t r)
{
  ...
  while(xx < yy)
  {
    ...
    /* Ҫ�жϵ�ǰ���Ƿ�����Ч��Χ�� */
    if((draw_x0 <= BTT_LCD_WIDTH) && (draw_y0 >= 0))
    {
    ...
    {  /* �������������ʼ��fill_x0 */
      ...
      /* �������������ʼ��fill_y0 */
      fill_y0 = draw_y1;
      if(fill_y0 > BTT_LCD_HEIGHT)
        fill_y0 = BTT_LCD_HEIGHT;
      if(fill_y0 < 0)
        fill_y0 = 0;
      /* �����������������fill_x1 */
      fill_x1 = x0*2 - draw_x1;
      if(fill_x1 > BTT_LCD_WIDTH)
        fill_x1 = BTT_LCD_WIDTH;
      GUI_HLine(fill_x0, fill_y0, fill_x1);
    }

    if((draw_x2 <= BTT_LCD_WIDTH) && (draw_y2 <= BTT_LCD_HEIGHT) )
    {
      GUI_DrawPoint(draw_x2, draw_y2);
    }

    if((draw_x3 >= 0) && (draw_y3 <= BTT_LCD_HEIGHT) )
    {
    ...
    if((draw_x4 <= BTT_LCD_WIDTH) && (draw_y4 >= 0))
    {
    ...
    if((draw_x6 <= BTT_LCD_WIDTH) && (draw_y6 <= BTT_LCD_HEIGHT))
    {
      GUI_DrawPoint(draw_x6, draw_y6);
    }

    if((draw_x7 >= 0) && (draw_y7 <= BTT_LCD_HEIGHT))
    {
