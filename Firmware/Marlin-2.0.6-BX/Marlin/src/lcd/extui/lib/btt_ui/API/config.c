...
const GUI_RECT  rectTitleline         = {0,               BYTE_HEIGHT+4,      BTT_LCD_WIDTH,                BYTE_HEIGHT+6};
const GUI_RECT  recterror             = {BYTE_WIDTH/2-2,  BYTE_HEIGHT*2+2,    BTT_LCD_WIDTH-BYTE_WIDTH/2+2, BTT_LCD_HEIGHT-(BYTE_HEIGHT*4)-4};
const GUI_RECT  recterrortxt          = {BYTE_WIDTH/2,    BYTE_HEIGHT*2+4,    BTT_LCD_WIDTH-BYTE_WIDTH/2,   BTT_LCD_HEIGHT-(BYTE_HEIGHT*4)-6};
const GUI_RECT  rectProgressframe     = {BYTE_WIDTH/2-2, BTT_LCD_HEIGHT-(BYTE_HEIGHT*2+BYTE_HEIGHT/2), BTT_LCD_WIDTH-BYTE_WIDTH/2+2,BTT_LCD_HEIGHT-BYTE_HEIGHT/2};
const GUI_POINT pointProgressText     = {BYTE_WIDTH/2-2, BTT_LCD_HEIGHT-(BYTE_HEIGHT*4)};
...
#define KEY_MAX_SIZE 2048
//check if keyword exits in the buffer line
static char key_seen(const char *keyStr)
{
  uint16_t i;
  for (c_index = 0; c_index < KEY_MAX_SIZE && cur_line[c_index] != 0; c_index++)
  {
...
{
  GUI_Clear(BLACK);
  GUI_DispString(2,2,(uint8_t*)"Updating Configuration...");
...
void drawProgress(void){
  ...
  // uint16_t p = map(configFile.cur,0,configFile.size, rectProgressframe.x0,rectProgressframe.x1);
...
void parseConfigKey(uint16_t index)
{
  ...
  case C_INDEX_LANGUAGE:
    if (inLimit(config_int(), 0, TSC_LANGUAGE_NUM-1))
  ...
  case C_INDEX_INVERT_AXIS:
    if (key_seen("X"))
        infoSettings.invert_axis[TSC_X_AXIS] = getOnOff();
    if (key_seen("Y"))
        infoSettings.invert_axis[TSC_Y_AXIS] = getOnOff();
    if (key_seen("Z"))
        infoSettings.invert_axis[TSC_Z_AXIS] = getOnOff();
    break;
  ...
  case C_INDEX_MODE:
    if (inLimit(config_int(), 0, LCD_MODE_COUNT-1))
  ...
  case C_INDEX_MAX_TEMP:
    ...
    { if (inLimit(config_int(), MIN_BED_TEMP, MAX_BED_TEMP))
        infoSettings.max_temp[TSC_BED] = config_int();
    }
    if (key_seen("CHAMBER:"))
    { if (inLimit(config_int(), MIN_CHAMBER_TEMP, MAX_CHAMBER_TEMP))
        infoSettings.max_temp[TSC_CHAMBER] = config_int();
    }
    if (key_seen("T0:"))
  ...
  case C_INDEX_BUILD_MIN:
    ...
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_min[TSC_X_AXIS] = config_int();
    }
    if (key_seen("Y"))
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_min[TSC_Y_AXIS] = config_int();
    }
    if (key_seen("Z"))
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_min[TSC_Z_AXIS] = config_int();
    }
    break;
  ...
  case C_INDEX_BUILD_MAX:
    if (key_seen("X"))
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_max[TSC_X_AXIS] = config_int();
    }
    if (key_seen("Y"))
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_max[TSC_Y_AXIS] = config_int();
    }
    if (key_seen("Z"))
    { if (inLimit(config_int(),  MIN_SIZE_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.machine_size_max[TSC_Z_AXIS] = config_int();
    }
    break;
  ...
  case C_INDEX_PAUSE_POS:
    ...
    { if (inLimit(config_float(), MIN_POS_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.pause_pos[TSC_X_AXIS] = config_float();
    }
    if (key_seen("Y"))
    { if (inLimit(config_float(), MIN_POS_LIMIT, MAX_SIZE_LIMIT))
        infoSettings.pause_pos[TSC_Y_AXIS] = config_float();
    }
    break;
