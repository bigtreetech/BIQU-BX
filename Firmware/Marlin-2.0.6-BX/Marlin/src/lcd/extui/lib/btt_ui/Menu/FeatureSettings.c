...
void updateFeatureSettings(uint8_t key_val)
{
  ...
  switch (item_index)
  {
    ...
    case SKEY_INVERT_X:
      infoSettings.invert_axis[TSC_X_AXIS] = (infoSettings.invert_axis[TSC_X_AXIS] + 1) % TOGGLE_NUM;
      settingPage[item_index].icon = toggleitem[infoSettings.invert_axis[TSC_X_AXIS]];
     break;

    case SKEY_INVERT_Y:
      infoSettings.invert_axis[TSC_Y_AXIS] = (infoSettings.invert_axis[TSC_Y_AXIS] + 1) % TOGGLE_NUM;
      settingPage[item_index].icon = toggleitem[infoSettings.invert_axis[TSC_Y_AXIS]];
      break;

    case SKEY_INVERT_Z:
      infoSettings.invert_axis[TSC_Z_AXIS] = (infoSettings.invert_axis[TSC_Z_AXIS] + 1) % TOGGLE_NUM;
      settingPage[item_index].icon = toggleitem[infoSettings.invert_axis[TSC_Z_AXIS]];
      break;
...
static SETTINGS now;
void menuFeatureSettingsCallBack(void)
{
  ...
  if((infoMenu.menu[infoMenu.cur] != menuFeatureSettings) && memcmp(&now, &infoSettings, sizeof(SETTINGS)))
  {
    storePara();
  }
  loopFrontEnd();
...
void menuFeatureSettings(void)
{
  ...
  menuSetFrontCallBack(menuFeatureSettingsCallBack);
}
