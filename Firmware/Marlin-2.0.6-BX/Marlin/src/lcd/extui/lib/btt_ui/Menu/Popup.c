...
#define X_MAX_CHAR (BTT_LCD_WIDTH / BYTE_WIDTH)
#define FULL_SCREEN_MAX_CHAR (BTT_LCD_WIDTH / BYTE_WIDTH * BTT_LCD_HEIGHT /BYTE_HEIGHT)
...
static void menuDialogCallBack(void)
{
...
{
  menuSetFrontCallBack(menuDialogCallBack);
}
...
{
  if (infoSettings.mode != LCD_MODE_SERIAL_TSC)
    return;
