...
void probeEnableOffset(void)
{
  ...
  mustStoreCmd("G1 X%.2f Y%.2f\n",
    -1 * getParameter(P_PROBE_OFFSET, X_STEPPER),
    -1 * getParameter(P_PROBE_OFFSET, Y_STEPPER));              // move nozzle to XY probing point
...
void showProbeOffsetStatus(uint8_t *status)
{
  char tempstr[20];
...
static float z_offset_value;
static float now;
static float max_unit;
void menuProbeOffsetCallBack(void)
{
  KEY_VALUES key_num = menuKeyGetValue();

  z_offset_value = getParameter(P_PROBE_OFFSET, Z_STEPPER);
  max_unit = probeOffset_unit[curUnit];
    switch (key_num)
    {
      // decrease offset
      case KEY_ICON_0:
...
now = z_offset_value = getParameter(P_PROBE_OFFSET, Z_STEPPER);
...
encoderPosition = 0;
  ...
  menuSetFrontCallBack(menuProbeOffsetCallBack);
}
